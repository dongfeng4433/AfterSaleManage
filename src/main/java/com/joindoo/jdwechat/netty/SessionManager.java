...
public class SessionManager {
	...
	public Session findByUserId(String userId) {
		String sessionId = this.userIdMap.get(userId);
		if (sessionId == null)
			return null;
		return this.findBySessionId(sessionId);
	}
	public synchronized Session put(String key, Session value) {
		...
		if(!Utility.isNullOrEmpty(value.getUserId())){
			this.userIdMap.put(value.getUserId(),value.getId());
		}
	...
	public synchronized Session removeBySessionId(String sessionId) {
		...
		if(!Utility.isNullOrEmpty(session.getUserId()))
			this.userIdMap.remove(session.getUserId());
		return session;
	}
