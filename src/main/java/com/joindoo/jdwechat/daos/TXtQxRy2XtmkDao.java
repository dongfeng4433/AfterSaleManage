...
public class TXtQxRy2XtmkDao extends BaseDao {
	public TXtQxRy2XtmkDao(){super();}
	public TXtQxRy2XtmkDao(DataContext dataContext){
		super(dataContext);
	}
  ...
  @Override
	public boolean exist(BaseModel model) {
	    String sql=((TXtQxRy2XtmkModel)model).getExist();
   ...
   public void insert(BaseModel model) {
       String sql=((TXtQxRy2XtmkModel)model).getInsert();
  ...
  public boolean insertOnSubmit(BaseModel model) throws Exception {
	TXtQxRy2XtmkModel bean= (TXtQxRy2XtmkModel)model;
  ...
  public void update(BaseModel model) {
	 String sql=((TXtQxRy2XtmkModel)model).getUpdate();
	 if(sql!=null&&sql!=""){
	 DBHelper.executeNonQuery(DataContext.getCurrentConnection(),sql);
	 }
  }
   public boolean updateOnSubmit(BaseModel model) throws Exception {
	 ...
	 if(changeFields.containsKey("sfsdtj_bj"))sb.append("sfsdtj_bj=?,");
	 if(changeFields.containsKey("sfjy_bj"))sb.append("sfjy_bj=?,");
	 ...
	 int index=1;
	 /*非主键*/
	 if(changeFields.containsKey("sfsdtj_bj")){
		 t=bean.getsfsdtj_bj();
		 if (t==null) statement.setNull(index, Types.INTEGER);
		 else statement.setObject(index, t, Types.INTEGER);
		 index++;
	 }
	 if(changeFields.containsKey("sfjy_bj")){
		 t=bean.getsfjy_bj();
		 if (t==null) statement.setNull(index, Types.INTEGER);
		 else statement.setObject(index, t, Types.INTEGER);
		 index++;
    ...
    @Override
    public TXtQxRy2XtmkModel find(BaseModel model) {
	 TXtQxRy2XtmkModel tModel=(TXtQxRy2XtmkModel)model;
   ...
   @Override
    public List<BaseModel> findAll(String condition) {
		TXtQxRy2XtmkModel tModel=new TXtQxRy2XtmkModel();
	...
	public void getData2ModelList(ResultSet set, List<BaseModel> resultList){
		try {
			if(null==set)return;
			while(set.next()){
 				TXtQxRy2XtmkModel model=new TXtQxRy2XtmkModel();
                model.fillModel(set);
	...
	public String getCountSql(String condition) {
		String sql="select count(*) as count from "+ TXtQxRy2XtmkModel.TABLE_NAME;
		return sql;
	}
   ...
   public void delete(BaseModel model) {
	 String sql=((TXtQxRy2XtmkModel)model).getDelete();
	 DBHelper.executeNonQuery(DataContext.getCurrentConnection(),sql);
  }
