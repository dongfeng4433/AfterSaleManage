...
public class TTxDxDxfsjlDao extends BaseDao {
	public TTxDxDxfsjlDao(){super();}
	public TTxDxDxfsjlDao(DataContext dataContext){
		super(dataContext);
	}
  ...
  @Override
	public boolean exist(BaseModel model) {
	    String sql=((TTxDxDxfsjlModel)model).getExist();
   ...
   public void insert(BaseModel model) {
       String sql=((TTxDxDxfsjlModel)model).getInsert();
  ...
  public boolean insertOnSubmit(BaseModel model) throws Exception {
	TTxDxDxfsjlModel bean= (TTxDxDxfsjlModel)model;
  ...
  public void update(BaseModel model) {
	 String sql=((TTxDxDxfsjlModel)model).getUpdate();
	 if(sql!=null&&sql!=""){
	 DBHelper.executeNonQuery(DataContext.getCurrentConnection(),sql);
	 }
  }
   public boolean updateOnSubmit(BaseModel model) throws Exception {
	 ...
	 if(changeFields.containsKey("nr"))sb.append("nr=?,");
	 if(changeFields.containsKey("fs_type"))sb.append("fs_type=?,");
	 ...
	 if(changeFields.containsKey("nr")){
		 t=bean.getnr();
		 if (t==null) statement.setNull(index, Types.VARCHAR);
		 else statement.setObject(index, t, Types.VARCHAR);
		 index++;
	 }
	 if(changeFields.containsKey("fs_type")){
		 t=bean.getfs_type();
		 if (t==null) statement.setNull(index, Types.VARCHAR);
		 else statement.setObject(index, t, Types.VARCHAR);
		 index++;
    ...
    @Override
    public TTxDxDxfsjlModel find(BaseModel model) {
	 TTxDxDxfsjlModel tModel=(TTxDxDxfsjlModel)model;
   ...
   @Override
    public List<BaseModel> findAll(String condition) {
		TTxDxDxfsjlModel tModel=new TTxDxDxfsjlModel();
	...
	public void getData2ModelList(ResultSet set, List<BaseModel> resultList){
		try {
			if(null==set)return;
			while(set.next()){
 				TTxDxDxfsjlModel model=new TTxDxDxfsjlModel();
                model.fillModel(set);
	...
	public String getCountSql(String condition) {
		String sql="select count(*) as count from "+ TTxDxDxfsjlModel.TABLE_NAME;
		return sql;
	}
   ...
   public void delete(BaseModel model) {
	 String sql=((TTxDxDxfsjlModel)model).getDelete();
	 DBHelper.executeNonQuery(DataContext.getCurrentConnection(),sql);
  }
