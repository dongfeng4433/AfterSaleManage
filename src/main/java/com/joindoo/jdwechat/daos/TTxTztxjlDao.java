...
public class TTxTztxjlDao extends BaseDao {
	public TTxTztxjlDao(){super();}
	public TTxTztxjlDao(DataContext dataContext){
		super(dataContext);
	}
  ...
  @Override
	public boolean exist(BaseModel model) {
	    String sql=((TTxTztxjlModel)model).getExist();
   ...
   public void insert(BaseModel model) {
       String sql=((TTxTztxjlModel)model).getInsert();
  ...
  public boolean insertOnSubmit(BaseModel model) throws Exception {
	TTxTztxjlModel bean= (TTxTztxjlModel)model;
  ...
  public void update(BaseModel model) {
	 String sql=((TTxTztxjlModel)model).getUpdate();
	 if(sql!=null&&sql!=""){
	 DBHelper.executeNonQuery(DataContext.getCurrentConnection(),sql);
	 }
  }
   public boolean updateOnSubmit(BaseModel model) throws Exception {
	 ...
	 if(changeFields.containsKey("bt"))sb.append("bt=?,");
	 if(changeFields.containsKey("tx_nr"))sb.append("tx_nr=?,");
	 if(changeFields.containsKey("tzry_sl"))sb.append("tzry_sl=?,");
	 if(changeFields.containsKey("sx_sj"))sb.append("sx_sj=?,");
	 if(changeFields.containsKey("fqry_xh"))sb.append("fqry_xh=?,");
	 if(changeFields.containsKey("xgjl_zj_xh"))sb.append("xgjl_zj_xh=?,");
	 ...
	 int index=1;
	 /*非主键*/
	 if(changeFields.containsKey("bt")){
		 t=bean.getbt();
		 if (t==null) statement.setNull(index, Types.VARCHAR);
		 else statement.setObject(index, t, Types.VARCHAR);
		 index++;
	 }
	 if(changeFields.containsKey("tx_nr")){
		 t=bean.gettx_nr();
		 if (t==null) statement.setNull(index, Types.VARCHAR);
		 else statement.setObject(index, t, Types.VARCHAR);
		 index++;
	 }
	 if(changeFields.containsKey("tzry_sl")){
		 t=bean.gettzry_sl();
		 if (t==null) statement.setNull(index, Types.INTEGER);
		 else statement.setObject(index, t, Types.INTEGER);
		 index++;
	 ...
	 if(changeFields.containsKey("fqry_xh")){
		 t=bean.getfqry_xh();
		 if (t==null) statement.setNull(index, Types.VARCHAR);
		 else statement.setObject(index, t, Types.VARCHAR);
		 index++;
	 }
	 if(changeFields.containsKey("xgjl_zj_xh")){
		 t=bean.getxgjl_zj_xh();
		 if (t==null) statement.setNull(index, Types.VARCHAR);
		 else statement.setObject(index, t, Types.VARCHAR);
		 index++;
    ...
    @Override
    public TTxTztxjlModel find(BaseModel model) {
	 TTxTztxjlModel tModel=(TTxTztxjlModel)model;
   ...
   @Override
    public List<BaseModel> findAll(String condition) {
		TTxTztxjlModel tModel=new TTxTztxjlModel();
	...
	public void getData2ModelList(ResultSet set, List<BaseModel> resultList){
		try {
			if(null==set)return;
			while(set.next()){
 				TTxTztxjlModel model=new TTxTztxjlModel();
                model.fillModel(set);
	...
	public String getCountSql(String condition) {
		String sql="select count(*) as count from "+ TTxTztxjlModel.TABLE_NAME;
		return sql;
	}
   ...
   public void delete(BaseModel model) {
	 String sql=((TTxTztxjlModel)model).getDelete();
	 DBHelper.executeNonQuery(DataContext.getCurrentConnection(),sql);
  }
