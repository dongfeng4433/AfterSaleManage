...
public class TDataEnterpriseOrderWorkflowDao extends BaseDao {
	public TDataEnterpriseOrderWorkflowDao(){super();}
	public TDataEnterpriseOrderWorkflowDao(DataContext dataContext){
		super(dataContext);
	}
  ...
  @Override
	public boolean exist(BaseModel model) {
	    String sql=((TDataEnterpriseOrderWorkflowModel)model).getExist();
   ...
   public void insert(BaseModel model) {
       String sql=((TDataEnterpriseOrderWorkflowModel)model).getInsert();
  ...
  public void update(BaseModel model) {
	 String sql=((TDataEnterpriseOrderWorkflowModel)model).getUpdate();
	 if(sql!=null&&sql!=""){
	 DBHelper.executeNonQuery(DataContext.getCurrentConnection(),sql);
	 }
  }
   public boolean updateOnSubmit(BaseModel model) throws Exception {
    ...
    @Override
    public TDataEnterpriseOrderWorkflowModel find(BaseModel model) {
	 TDataEnterpriseOrderWorkflowModel tModel=(TDataEnterpriseOrderWorkflowModel)model;
   ...
   @Override
    public List<BaseModel> findAll(String condition) {
		TDataEnterpriseOrderWorkflowModel tModel=new TDataEnterpriseOrderWorkflowModel();
	...
	public void getData2ModelList(ResultSet set, List<BaseModel> resultList){
		try {
			if(null==set)return;
			while(set.next()){
 				TDataEnterpriseOrderWorkflowModel model=new TDataEnterpriseOrderWorkflowModel();
                model.fillModel(set);
	...
	public String getCountSql(String condition) {
		String sql="select count(*) as count from "+ TDataEnterpriseOrderWorkflowModel.TABLE_NAME;
		return sql;
	}
   ...
   public void delete(BaseModel model) {
	 String sql=((TDataEnterpriseOrderWorkflowModel)model).getDelete();
	 DBHelper.executeNonQuery(DataContext.getCurrentConnection(),sql);
  }
