...
public class TXtQxXtjs2XtmkDao extends BaseDao {
	public TXtQxXtjs2XtmkDao(){super();}
	public TXtQxXtjs2XtmkDao(DataContext dataContext){
		super(dataContext);
	}
  ...
  @Override
	public boolean exist(BaseModel model) {
	    String sql=((TXtQxXtjs2XtmkModel)model).getExist();
   ...
   public void insert(BaseModel model) {
       String sql=((TXtQxXtjs2XtmkModel)model).getInsert();
  ...
  public boolean insertOnSubmit(BaseModel model) throws Exception {
	TXtQxXtjs2XtmkModel bean= (TXtQxXtjs2XtmkModel)model;
  ...
  public void update(BaseModel model) {
	 String sql=((TXtQxXtjs2XtmkModel)model).getUpdate();
	 if(sql!=null&&sql!=""){
	 DBHelper.executeNonQuery(DataContext.getCurrentConnection(),sql);
	 }
  }
   public boolean updateOnSubmit(BaseModel model) throws Exception {
    ...
    @Override
    public TXtQxXtjs2XtmkModel find(BaseModel model) {
	 TXtQxXtjs2XtmkModel tModel=(TXtQxXtjs2XtmkModel)model;
   ...
   @Override
    public List<BaseModel> findAll(String condition) {
		TXtQxXtjs2XtmkModel tModel=new TXtQxXtjs2XtmkModel();
	...
	public void getData2ModelList(ResultSet set, List<BaseModel> resultList){
		try {
			if(null==set)return;
			while(set.next()){
 				TXtQxXtjs2XtmkModel model=new TXtQxXtjs2XtmkModel();
                model.fillModel(set);
	...
	public String getCountSql(String condition) {
		String sql="select count(*) as count from "+ TXtQxXtjs2XtmkModel.TABLE_NAME;
		return sql;
	}
   ...
   public void delete(BaseModel model) {
	 String sql=((TXtQxXtjs2XtmkModel)model).getDelete();
	 DBHelper.executeNonQuery(DataContext.getCurrentConnection(),sql);
  }
