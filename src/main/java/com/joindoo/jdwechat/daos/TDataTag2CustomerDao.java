...
public class TDataTag2CustomerDao extends BaseDao {
	public TDataTag2CustomerDao(){super();}
	public TDataTag2CustomerDao(DataContext dataContext){
		super(dataContext);
	}
  ...
  @Override
	public boolean exist(BaseModel model) {
	    String sql=((TDataTag2CustomerModel)model).getExist();
   ...
   public void insert(BaseModel model) {
       String sql=((TDataTag2CustomerModel)model).getInsert();
  ...
  public boolean insertOnSubmit(BaseModel model) throws Exception {
	TDataTag2CustomerModel bean= (TDataTag2CustomerModel)model;
  ...
  public void update(BaseModel model) {
	 String sql=((TDataTag2CustomerModel)model).getUpdate();
	 if(sql!=null&&sql!=""){
	 DBHelper.executeNonQuery(DataContext.getCurrentConnection(),sql);
	 }
  }
   public boolean updateOnSubmit(BaseModel model) throws Exception {
    ...
    @Override
    public TDataTag2CustomerModel find(BaseModel model) {
	 TDataTag2CustomerModel tModel=(TDataTag2CustomerModel)model;
   ...
   @Override
    public List<BaseModel> findAll(String condition) {
		TDataTag2CustomerModel tModel=new TDataTag2CustomerModel();
	...
	public void getData2ModelList(ResultSet set, List<BaseModel> resultList){
		try {
			if(null==set)return;
			while(set.next()){
 				TDataTag2CustomerModel model=new TDataTag2CustomerModel();
                model.fillModel(set);
	...
	public String getCountSql(String condition) {
		String sql="select count(*) as count from "+ TDataTag2CustomerModel.TABLE_NAME;
		return sql;
	}
   ...
   public void delete(BaseModel model) {
	 String sql=((TDataTag2CustomerModel)model).getDelete();
	 DBHelper.executeNonQuery(DataContext.getCurrentConnection(),sql);
  }
