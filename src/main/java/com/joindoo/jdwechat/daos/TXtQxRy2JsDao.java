...
public class TXtQxRy2JsDao extends BaseDao {
	public TXtQxRy2JsDao(){super();}
	public TXtQxRy2JsDao(DataContext dataContext){
		super(dataContext);
	}
  ...
  @Override
	public boolean exist(BaseModel model) {
	    String sql=((TXtQxRy2JsModel)model).getExist();
   ...
   public void insert(BaseModel model) {
       String sql=((TXtQxRy2JsModel)model).getInsert();
  ...
  public boolean insertOnSubmit(BaseModel model) throws Exception {
	TXtQxRy2JsModel bean= (TXtQxRy2JsModel)model;
  ...
  public void update(BaseModel model) {
	 String sql=((TXtQxRy2JsModel)model).getUpdate();
	 if(sql!=null&&sql!=""){
	 DBHelper.executeNonQuery(DataContext.getCurrentConnection(),sql);
	 }
  }
   public boolean updateOnSubmit(BaseModel model) throws Exception {
    ...
    @Override
    public TXtQxRy2JsModel find(BaseModel model) {
	 TXtQxRy2JsModel tModel=(TXtQxRy2JsModel)model;
   ...
   @Override
    public List<BaseModel> findAll(String condition) {
		TXtQxRy2JsModel tModel=new TXtQxRy2JsModel();
	...
	public void getData2ModelList(ResultSet set, List<BaseModel> resultList){
		try {
			if(null==set)return;
			while(set.next()){
 				TXtQxRy2JsModel model=new TXtQxRy2JsModel();
                model.fillModel(set);
	...
	public String getCountSql(String condition) {
		String sql="select count(*) as count from "+ TXtQxRy2JsModel.TABLE_NAME;
		return sql;
	}
   ...
   public void delete(BaseModel model) {
	 String sql=((TXtQxRy2JsModel)model).getDelete();
	 DBHelper.executeNonQuery(DataContext.getCurrentConnection(),sql);
  }
